...
<div>
	<x-admin-panel>
		<x-slot name="header">{{ __('Admins') }}</x-slot>
		<x-slot name="desc">
			{{ __('Manage the admin users in the system.') }}
			{{ __('You can create, edit, or delete admins here.') }}
		</x-slot>
		<x-slot name="button">
			...
			<table class="min-w-full divide-y divide-gray-300 dark:divide-gray-600">
				<thead class="bg-gray-50 dark:bg-gray-700">
					<tr>
						...
						<th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">Phone</th>
						<th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">WhatsApp</th>
					...
					@if ($this->results->isNotEmpty())
						@foreach ($this->results as $row)
							<tr class="even:bg-gray-50 dark:even:bg-gray-800/50">
								...
								<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $row->name }}</td>
								<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $row->email }}</td>
								<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $row->phone ?? '-' }}</td>
								<td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $row->getMeta('whatsapp_number') ?? '-' }}</td>
		...
		<script>
			...
			$js('confirmDelete', (id) => {
				Swal.fire({
					...
					text: "This admin will be removed permanently.",
					icon: 'warning',
