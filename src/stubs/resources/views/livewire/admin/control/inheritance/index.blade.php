...
new class extends Component {
    ...
    public function results()
    {
        ...
        // Filter by inheritance group
        $query->where('group', 'inheritance');
    ...
    public function activate($id)
    {
        ...
        if ($category) {
            ...
            $this->notifySuccess(__('Inheritance was activated successfully!'));
        } else {
            $this->notifyError(__('Inheritance not found!'));
        }
    }
    ...
    public function deactivate($id)
    {
        ...
        if ($category) {
            ...
            $this->notifySuccess(__('Inheritance was deactivated successfully!'));
        } else {
            $this->notifyError(__('Inheritance not found!'));
        }
    }
    ...
    public function delete($id)
    {
        try {
            ...
            $this->notifyError(__('Failed to delete inheritance:' . $e->getMessage()));
        }
    }
}; ?>
...
<div>
	{{-- @dd($results) --}}
	<x-admin-panel>
		<x-slot name="header">{{ __('Inheritance') }}</x-slot>
		<x-slot name="desc">
			{{ __('Manage the inheritance displayed in the mobile app.') }}
						...
						@foreach ($this->results as $row)
							<tr class="even:bg-gray-50 dark:even:bg-gray-800/50">
								...
								<td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-3">
									<a href="{{ route('admin.inheritance.edit', $row->id) }}"
		...
		<script>
			...
			$js('confirmDelete', (id) => {
				Swal.fire({
					...
					text: "This inheritance will be removed permanently.",
					icon: 'warning',
