...
new class extends Component {
    ...
    public $search = '';
    public $perPage = 10;
    public $group = 'country'; // Default group
    ...
    public function activate($id): void
    {
        ...
        if ($location) {
            ...
            $this->notifySuccess(__('Location was activated successfully!'));
        } else {
            $this->notifyError(__('Location not found!'));
        }
    }
    ...
    public function deactivate($id): void
    {
        ...
        if ($location) {
            ...
            $this->notifySuccess(__('Location was deactivated successfully!'));
        } else {
            $this->notifyError(__('Location not found!'));
        }
    }
    ...
    public function delete($id): void
    {
        try {
            ...
            $this->notifyError(__('Failed to delete location:' . $e->getMessage()));
        }
    }
}; ?>
...
<div>
    <x-admin-panel>
        <x-slot name="header">{{ __('' . ucfirst($this->group) . ' Locations') }}</x-slot>
        <x-slot name="desc">
            {{ __('Manage the ' . $this->group . ' locations displayed in the application.') }}
            {{ __('You can create, edit, enable/disable, or delete ' . $this->group . ' locations here.') }}
        </x-slot>
        <x-slot name="button">
            @php
                $createRoute = match($this->group) {
                    'country' => 'admin.countries.create',
                    'city' => 'admin.cities.create',
                    'area' => 'admin.areas.create',
                    'zone' => 'admin.zones.create',
                    default => 'admin.countries.create'
                };
            @endphp
                    ...
                    <form class="sm:flex sm:items-center">
                        <div class="w-full sm:max-w-xs">
                            <input type="text" wire:model.live.debounce.300ms="search"
                                ...
                                placeholder="Search {{ $this->group }} locations..." />
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="overflow-hidden shadow-sm ring-1 ring-black/5 dark:ring-white/10 sm:rounded-lg mt-2">
            ...
            <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-600">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        @if ($this->group === 'country')
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">Dial Code</th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-gray-800">
                    @if ($this->results->isNotEmpty())
                        @foreach ($this->results as $row)
                            <tr class="even:bg-gray-50 dark:even:bg-gray-800/50">
                                ...
                                @if ($row->type == 'location' && $this->group === 'country')
                                    <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $row->getMeta('dial_code', 'N/A') }}</td>
                                @endif
                                ...
                                <td class="relative py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-3">
                                    @php
                                        $editRoute = match($this->group) {
                                            'country' => 'admin.countries.edit',
                                            'city' => 'admin.cities.edit',
                                            'area' => 'admin.areas.edit',
                                            'zone' => 'admin.zones.edit',
                                            default => 'admin.countries.edit'
                                        };
                                    @endphp
                    ...
                    @else
                        <tr class="even:bg-gray-50 dark:even:bg-gray-800/50">
                            <td colspan="{{ $this->group === 'country' ? '7' : '6' }}"
